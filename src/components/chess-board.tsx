...
export default function ChessBoard() {
	...
	return (
		<div className="flex w-full flex-col items-center gap-5">
			...
			<div className="border-border relative aspect-square w-full max-w-[560px] overflow-hidden rounded-lg border shadow-2xl shadow-black/60 select-none">
								...
								<button
									...
									className={cn(
										...
										light
											...
											: "bg-[oklch(0.22_0_0)]",
										isSelected &&
											"ring-accent z-10 ring-2 ring-inset",
									)}
								>
									{isTarget && !isCapture && (
										<span className="bg-accent/70 absolute h-3 w-3 rounded-full" />
									)}
									{isCapture && (
										<span className="ring-accent/80 absolute inset-1 rounded-md ring-2" />
									)}
									{cell && (
										<span
											aria-hidden
											className={cn(
												"relative font-sans leading-none select-none",
												cell.color === "w"
			...
			<AnimatePresence>
				{solved && (
					<motion.div
						...
						className="border-accent/50 bg-accent/10 text-foreground rounded-full border px-5 py-2 font-mono text-sm backdrop-blur"
					>
						gg. you found it.
					</motion.div>
				)}
